...
| Before          | After
|-----------------|----------------------------------------
| CallOROut | Your account or corporation name
| Genemed      | Your own project name
| AUTHOR       | Author name
...
Also as you can see from the [package.json](package.json) file, this project requires the private npm module `@CallOROut`, provided from the Github. Therefore, to develop this backend server, you've configure the `.npmrc` file. Open the below link and complete the configuration.
...
### 4.2. Directories
  ...
  - [src/](src/): TypeScript Source directory
    - [src/api/](src/api/): Client SDK that would be published to the `@CallOROut/Genemed-api`
